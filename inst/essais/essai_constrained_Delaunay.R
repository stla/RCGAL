...
delCDT <- RCDT::delaunay(points, edges = edges)
delCDT$triangles <- del$faces
RCDT::plotDelaunay(delCDT)


library(microbenchmark)
microbenchmark(
  CGAL = delaunay(points, constraints = edges),
  CDT  = RCDT::delaunay(points, edges = edges),
  times = 10
)
